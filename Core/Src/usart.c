...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	{
//      if( ReadBlockData(ballID) == STATUS_OK)
//				printf("ok\n");
//		
	}
	...
	{
//		if(Get_qrcode(&qrcode) == 0x01)
//		{
//			send_qrcode(&qrcode);
//		}
//        HAL_UART_Transmit(&huart1, &qrcode_Rx_buffer, 1, 0xff);
		if(qrcode.last_message != qrcode_Rx_buffer)
		{
			qrcode.message=qrcode_Rx_buffer;
		}
		qrcode.last_message=qrcode.message;
	...
	if(huart->Instance==USART3)
    {
        //HAL_UART_Transmit(&huart1, &Buffer, 1, 0xff);
				...
				Get_openmv(&openmv);
        HAL_UART_Receive_IT(&huart3,OPENMV_Rx_BUF,5);
    }
